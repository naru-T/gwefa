gwefa.cv_uniquenesses.calc <- function(bw, x, dp.locat,k, scores, robust, kernel, adaptive, p, theta, longlat, dMat,
 ...
 ##This function is based on GWmodel::gwpca.cv.
  ...
  for (i in 1:ep.n) {
    ...
    wt[i] <- 0
    temp1 <- wefa(x=data, wt, factors=k, scores=scores, n.obs, fm, rotate)
