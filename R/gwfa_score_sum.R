gwfa.score_dif.calc <- function(bw, x ,dp.locat, k, robust, kernel, adaptive, p, theta, longlat, dMat,vars, n.obs, fm, rotate,scores, oblique.scores, timeout, foreach){
  ...
  temp1 <- gwfa(data=x,elocat=dp.locat, vars,bw,k, kernel, adaptive, p=2, theta=0, longlat, dMat, n.obs = n.obs, fm, rotate,scores, oblique.scores=oblique.scores, timeout=timeout, foreach=foreach)
